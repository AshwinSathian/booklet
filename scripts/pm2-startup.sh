#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# pm2-startup.sh — Booklet PM2 startup watchdog
#
# Run by com.booklet.pm2.plist at login. Ensures booklet-app and
# booklet-mcp are registered and running in PM2. PM2's own autorestart
...
# slot — just ensures booklet processes are present.
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail

NODE_BIN="$HOME/.nvm/versions/node/v20.19.5/bin"
PM2="$NODE_BIN/pm2"
ECOSYSTEM="/Users/ashwinsathian/Documents/Personal/booklet/ecosystem.config.js"
POLL_INTERVAL=15
PING_TIMEOUT_SEC=5
LOG_DIR="$HOME/.booklet/logs"
mkdir -p "$LOG_DIR"
...
log() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] [booklet-pm2] $*"; }
...
ensure_booklet() {
  ...
  for svc in booklet-app booklet-mcp; do
